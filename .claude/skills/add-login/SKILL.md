...
- **CRITICO**: si el proyecto usa estructura `src/` (app en `src/app`), `proxy.ts` DEBE vivir en `src/proxy.ts`, NUNCA en la raiz del repo. Next.js lo ignora en silencio si esta en el lugar equivocado — sin error de build, sin warning, todas las rutas quedan desprotegidas. Verificar SIEMPRE con `ls src/app` antes de decidir donde crear `proxy.ts`: si existe `src/app`, crearlo en `src/proxy.ts`; si no, en la raiz.
- Validar la proteccion con `curl -s -o /dev/null -w "%{http_code} %{redirect_url}" http://localhost:PUERTO/dashboard` (sin cookies) — debe devolver `307` hacia `/login`. NUNCA dar por validada la proteccion solo con clicks de navegador con sesion activa (puede ocultar el bug).
...
### 1. `proxy.ts` (`src/proxy.ts` si el proyecto usa `src/app`; si no, en la raiz — ver CRITICO arriba)
