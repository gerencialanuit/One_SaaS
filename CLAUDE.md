...
### 2026-08-12: Policy RLS que consulta su propia tabla causa recursion infinita (y el error se puede tragar en silencio)
- **Error**: CRITICO. Una policy RLS del tipo "el rol X ve todas las filas" que hace `exists (select 1 from esta_misma_tabla where ...)` dentro de su propia definicion causa `infinite recursion detected in policy`. Si el codigo que consume esa tabla solo desestructura `data` de la respuesta de Supabase sin revisar `error` (patron `const { data } = await supabase.from(...)`), el error de recursion se traga en silencio y la funcion devuelve `null`/vacio como si no hubiera datos — cualquier logica de permisos basada en ese resultado falla silenciosamente a "denegado", y es dificil de notar porque los casos NEGATIVOS (ocultar algo a quien no tiene el rol) se ven identicos a un error silencioso.
- **Fix**: Cualquier policy que necesite "ver todas las filas segun el rol" de una tabla debe usar una funcion `SECURITY DEFINER` externa (ej. `current_user_role()`) en vez de una subconsulta directa sobre la tabla que protege. Y CUALQUIER codigo que llame a Supabase debe revisar `error`, nunca desestructurar solo `data`.
- **Aplicar en**: Todos los proyectos de la fabrica con RLS por rol. Al crear una tabla con roles (ej. `profiles` con `add-login` extendido), escribir la policy de "admin ve todo" con una funcion helper desde el inicio, no con una subconsulta directa. Validar cualquier chequeo de permisos con un caso POSITIVO real, no solo negativo.

### 2026-08-12: NUNCA revocar EXECUTE de 'authenticated' en una funcion helper usada dentro de policies RLS
- **Error**: CRITICO. Al usar un patron de funcion helper `SECURITY DEFINER` (ej. `current_user_role()`) referenciada DENTRO de policies RLS (`using (current_user_role() = 'gerente')`), revocar `EXECUTE` de esa funcion para el rol `authenticated` (para "cerrar" un warning del security advisor sobre exposicion via RPC) rompe SILENCIOSAMENTE todas las queries de usuarios logueados que disparen esa policy — error `permission denied for function <nombre>`. Es dificil de detectar: `curl` sin cookies solo prueba el proxy/middleware, nunca ejecuta una query real contra RLS.
- **Fix**: Solo revocar `EXECUTE` de `anon` (y `public`) para funciones asi. `authenticated` SIEMPRE necesita `EXECUTE` si la funcion se usa dentro de una policy RLS que aplica a usuarios logueados — aunque eso la deje tambien invocable como RPC publico por cualquier usuario logueado (generalmente inofensivo si la funcion solo devuelve datos del propio llamador).
- **Aplicar en**: Todos los proyectos con RLS que usen un patron `current_user_role()`/`current_user_permission()` o similar. Antes de revocar EXECUTE de cualquier funcion SECURITY DEFINER por un warning del advisor, buscar primero en que policies se usa (`grep` las migraciones por el nombre de la funcion). Verificar el fix SIEMPRE con una query autenticada simulada (`set local role authenticated; select set_config('request.jwt.claims', ...)`), nunca solo con curl sin cookies.

### 2026-08-08: proxy.ts en la raiz es ignorado si el proyecto usa src/
- **Error**: CRITICO. El skill `add-login` genera `proxy.ts` en la raiz del repo. Si el proyecto usa estructura `src/` (app en `src/app`, que es el default de esta fabrica), Next.js 16 IGNORA `proxy.ts` en silencio si no esta dentro de `src/` — sin error de build ni de dev server. Resultado: cero rutas protegidas, cualquiera accede sin sesion. No se detecta con clicks de navegador (la sesion del navegador oculta el bug), solo con `curl` sin cookies.
